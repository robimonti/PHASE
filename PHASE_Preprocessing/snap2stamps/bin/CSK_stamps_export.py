...
if coreg_files:
    first_file = os.path.basename(coreg_files[0])
    master_date = first_file[:8] # Grabs the first 8 digits (Master Date)
else:
    master_date = "UNKNOWN"

outputexportfolder = os.path.join(PROJECT, f'INSAR_{master_date}')
...
outlog=logfolder+'/CSK_export_proc_stdout.log'
out_file = open(outlog, 'a')
err_file=out_file
graphxml=GRAPH+'/CSK_export.xml'
graph2run=PROJECT+'/graphs/CSK_export2run.xml'
print(bar_message)
