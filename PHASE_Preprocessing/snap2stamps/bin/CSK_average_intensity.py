### Python script to compute Full-Stack Average Intensity for CSK
# Upgraded to bypass SNAP 9 BandMerge conflicts by calculating Intensity pre-merge
...
outlog = os.path.join(logfolder, 'CSK_avg_intensity_proc_stdout.log')
graphxml = os.path.join(graphs_dir, 'CSK_stack_intensity.xml')
...
# The Master is the ONLY band that appears in every single coregistered pair.
# We intersect the sets to mathematically guarantee we find the Master.
...
# Calculate Master Intensity (only needs to be done once, pulled from the first pair)
xml_content += f'''
  <node id="Math_Master">
...
# Calculate all Slave Intensities
for index, file in enumerate(coreg_files):
...
# Merge all pre-calculated intensities safely
merge_sources_str = "\n".join(merge_sources)
xml_content += f'''
  <node id="BandMerge">
...
xml_content += f'''
  <node id="Terrain-Correction">
    ...
    <parameters>
      ...
      <pixelSpacingInMeter>3.0</pixelSpacingInMeter>
