...
def format_excel_sheet4(report_path, figure_path, sheet_name):
    # Load the workbook and select the sheet
    wb = openpyxl.load_workbook(report_path)
    sheet = wb[sheet_name]

    # Insert the figure
    figure_position = "A7"  # Start of figure space
    ...
    subtitle_row = 31  # Start of the table (below figure space)
    ...
    subtitle_row = 31  # Subtitle row
    data_start_row = subtitle_row + 2  # Data starts two rows after the subtitle
    for row in sheet.iter_rows(min_row=data_start_row, max_row=sheet.max_row, min_col=2, max_col=sheet.max_column):
        for cell in row:
            if cell.value:
                cell.font = Font(size=12)
                cell.alignment = Alignment(horizontal='left', vertical='center')
    ...
    # Format the third subtitle (A4)
    sheet['A4'].font = Font(size=16, bold=False)
    sheet['A4'].alignment = Alignment(horizontal='left', vertical='center')

    # Format the fourth subtitle (A31)
    sheet[f'A{subtitle_row}'].font = Font(size=18, bold=True)
    sheet[f'A{subtitle_row}'].alignment = Alignment(horizontal='left', vertical='center')
...
if __name__ == "__main__":
    ...
    format_excel_sheet4(report_file, figure_file, sheet_name)
