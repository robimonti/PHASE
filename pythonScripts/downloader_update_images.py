...
With embedded urls: ( download the hardcoded list of files in the 'files =' block below)
    ...
    python ./download-all-2026-05-27_09-03-19.py

Download all files in a Metalink/CSV: (downloaded from ASF Vertex)

    python ./download-all-2026-05-27_09-03-19.py /path/to/downloads.metalink localmetalink.metalink localcsv.csv
...
class bulk_downloader:
    def __init__(self):
        # List of files to download. The GUI populates this from selected images.
        self.files = []
        self.output_json_path = None
        self.repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
        self.credentials_path = os.path.join(self.repo_root, 'downloadasf', 'login_request.json')
        ...
        if '--help' in sys.argv or '-h' in sys.argv:
            print("Usage: python downloader_update_images.py --files selected_urls.txt --destination /path/to/slaves [--output summary.json] [--insecure]")
            print("Also accepts ASF .metalink and .csv files, matching the ASF bulk download script.")
            exit(0)

        # Check if user handed in a Metalink/CSV/TXT or GUI-provided options.
        if len(sys.argv) > 1:
            download_files = []
            input_files = []
            args = sys.argv[1:]
            ...
            while i < len(args):
                ...
                if arg == '--insecure':
                ...
                elif arg in ('--destination', '-d'):
                    if i + 1 >= len(args):
                        print(" > Missing value for --destination")
                        exit(-1)
                    ...
                    if i + 1 >= len(args):
                        ...
                        exit(-1)
                    self.output_json_path = args[i + 1]
                    i += 2

                elif arg in ('--files', '--urls-file', '-f'):
                    if i + 1 >= len(args):
                    ...
                    if os.path.isfile(input_file):
                        ...
                        new_files = self.process_txt(input_file)
                        if new_files is not None:
                            for file_url in new_files:
                                download_files.append(file_url)
                    else:
                        print(f" > I cannot find the input file you specified: {input_file}")
                    i += 2
                    ...
                    if os.path.isfile(arg):
                        input_files.append(arg)
                        new_files = self.process_txt(arg)
                        if new_files is not None:
                            for file_url in new_files:
                                download_files.append(file_url)
                    else:
        ...
        if len(self.files) == 0:
            print(" > No download URLs were provided. Use --files selected_urls.txt.")
            exit(-1)
    ...
    def load_saved_credentials(self):
        ...
        try:
            with open(self.credentials_path, 'r', encoding='utf-8') as stream:
                data = json.load(stream)
        except (OSError, json.JSONDecodeError) as exc:
            print(f" > Could not read saved ASF credentials from {self.credentials_path}: {exc}")
            return None

        username = str(data.get('username', '')).strip()
        password = str(data.get('password', ''))
        if not username or not password:
            print(f" > Saved ASF credentials file is incomplete: {self.credentials_path}")
            return None
    ...
    def get_cookie(self):
        ...
        if saved_credentials is not None:
            print(f" > Trying saved Earthdata credentials from {self.credentials_path}")
            if self.get_new_cookie(*saved_credentials, prompt=False) and self.check_cookie():
                return
            print(" > Saved Earthdata credentials were not accepted; falling back to interactive prompt.")

        # We don't have a valid cookie, prompt user.
        print("No existing URS cookie found, please enter Earthdata username & password:")
        print("(Credentials are read from downloadasf/login_request.json when available; otherwise they are not stored by this script)")
    ...
    def check_cookie(self):
        ...
        try:
            ...
            print(f" > Could not validate the existing URS cookie: {e}")
            print(" > Check your internet connection, DNS/proxy settings, or try again later.")
            return False
    ...
    def get_new_cookie(self, username=None, password=None, prompt=True):
        # Start by prompting user to input their credentials when no saved credentials are provided.

        if prompt or username is None or password is None:
        ...
        try:
            ...
            print(f"Try cookie generation later. Details: {e}")
            exit(-1)
    ...
    # Get download urls from a plain text file, one URL per line.
    def process_txt(self, txt_file):
        print(f"Processing URL list file: {txt_file}")
...
if __name__ == "__main__":
    ...
    downloader.write_json_summary()

    if len(downloader.failed) > 0:
        sys.exit(1)
